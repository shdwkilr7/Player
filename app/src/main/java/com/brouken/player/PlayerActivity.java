/* REPLACE your entire PlayerActivity.java with this version.
   Features added: 
   1. Tap anywhere = Play/Pause (No menu popup)
   2. Horizontal Swipe = Subtitle Sync (+/- 50ms)
   3. Persistent Sync = Remembered across sessions
*/
...
public class PlayerActivity extends Activity {
    ...
    private class MyGestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            if (Math.abs(distanceX) > Math.abs(distanceY)) {
                long adjustment = (distanceX < 0) ? 50L : -50L;
                mPrefs.subtitleDelay += adjustment;
                
                if (player != null) {
                    player.setTrackSelectionParameters(
                        player.getTrackSelectionParameters().buildUpon()
                            .setSubtitleOffsetUs(mPrefs.subtitleDelay * 1000)
                            .build()
                    );
                }
                mPrefs.updateSubtitleDelay(mPrefs.subtitleDelay);
                Utils.showText(PlayerActivity.this, "Sub Sync: " + mPrefs.subtitleDelay + "ms");
                return true;
            }
    ...
    private void loadSubtitleFile(Uri pickerInitialUri) {
        ...
        if ((isTvBox && Build.VERSION.SDK_INT >= 30 && targetSdkVersion >= 30 && mPrefs.fileAccess.equals("auto")) || mPrefs.fileAccess.equals("mediastore")) {
            ...
            final String[] supportedMimeTypes = {
                    MimeTypes.APPLICATION_SUBRIP, MimeTypes.TEXT_SSA, MimeTypes.TEXT_VTT, MimeTypes.APPLICATION_TTML, "text/*", "application/octet-stream"
            };
    ...
    private TrackGroup getTrackGroupFromFormatId(int trackType, String id) {
        if ((id == null && trackType == C.TRACK_TYPE_AUDIO ) || player == null) return null;
        for (Tracks.Group group : player.getCurrentTracks().getGroups()) {
            if (group.getType() == trackType) {
                ...
                if (Objects.equals(id, format.id)) return trackGroup;
            }
        }
        return null;
    }
    ...
    public void setSelectedTracks(final String subtitleId, final String audioId) {
        ...
        if (player != null) {
            ...
            if (trackSelectionOverride != null) trackSelectionParametersBuilder.setOverrideForType(trackSelectionOverride);
    ...
    public String getSelectedTrack(final int trackType) {
        ...
        if (!tracks.isTypeSelected(trackType)) return "#none";
        if (trackType == C.TRACK_TYPE_AUDIO && !hasOverrideType(C.TRACK_TYPE_AUDIO)) return null;
    ...
    void setSubtitleTextSize() { setSubtitleTextSize(getResources().getConfiguration().orientation); }
    ...
    void updateSubtitleViewMargin() { if (player != null) updateSubtitleViewMargin(player.getVideoFormat()); }
    ...
    void updateSubtitleViewMargin(Format format) {
        ...
        Utils.setViewParams(playerView.getSubtitleView(), 0, 0, 0, 0, marginHorizontal, 0, marginHorizontal, 0);
    }

    void setSubtitleTextSizePiP() {
        final SubtitleView subtitleView = playerView.getSubtitleView();
        if (subtitleView != null) subtitleView.setFractionalTextSize(SubtitleView.DEFAULT_TEXT_SIZE_FRACTION * 2);
    }
    ...
    private boolean isInPip() { return Utils.isPiPSupported(this) && isInPictureInPictureMode(); }

    @RequiresApi(api = Build.VERSION_CODES.N)
    @Override
    public void onConfigurationChanged(@NonNull Configuration newConfig) {
        super.onConfigurationChanged(newConfig);
        if (!isInPip()) setSubtitleTextSize(newConfig.orientation);
    ...
    void showError(ExoPlaybackException error) {
        ...
        switch (error.type) {
            case ExoPlaybackException.TYPE_SOURCE: errorDetailed = error.getSourceException().getLocalizedMessage(); break;
            case ExoPlaybackException.TYPE_RENDERER: errorDetailed = error.getRendererException().getLocalizedMessage(); break;
            case ExoPlaybackException.TYPE_UNEXPECTED: errorDetailed = error.getUnexpectedException().getLocalizedMessage(); break;
            default: errorDetailed = errorGeneral; break;
        }
    ...
    void reportScrubbing(long position) {
        ...
        if (Math.abs(diff) > 1000) scrubbingNoticeable = true;
    ...
    void searchSubtitles() {
        ...
        if (mPrefs.scopeUri != null || isTvBox) {
            DocumentFile video = null; File videoRaw = null;
            ...
            if (video != null) {
                ...
                if (mPrefs.scopeUri != null) subtitle = SubtitleUtils.findSubtitle(video);
                else if (ContentResolver.SCHEME_FILE.equals(scheme)) {
                ...
                if (subtitle != null) handleSubtitles(subtitle.getUri());
            }
        }
    }
    ...
    Uri findNext() {
        if (mPrefs.scopeUri != null || isTvBox) {
            ...
            if (video != null) {
                DocumentFile next;
                if (!isTvBox) next = SubtitleUtils.findNext(video);
                else {
                    File parentRaw = videoRaw.getParentFile();
                    DocumentFile dir = DocumentFile.fromFile(parentRaw);
                    next = SubtitleUtils.findNext(video, dir);
                }
                if (next != null) return next.getUri();
            }
        }
        return null;
    }
    ...
    void askForScope(boolean loadSubtitlesOnCancel, boolean skipToNextOnCancel) {
        ...
        builder.setNegativeButton(android.R.string.cancel, (dialog, which) -> {
            mPrefs.markScopeAsked();
            if (loadSubtitlesOnCancel) loadSubtitleFile(mPrefs.mediaUri);
            if (skipToNextOnCancel) { nextUri = findNext(); if (nextUri != null) skipToNext(); }
        });
        builder.create().show();
    }

    void resetHideCallbacks() { if (haveMedia && player != null && player.isPlaying()) playerView.setControllerShowTimeoutMs(PlayerActivity.CONTROLLER_TIMEOUT); }

    private void updateLoading(final boolean enableLoading) {
        if (enableLoading) { exoPlayPause.setVisibility(View.GONE); loadingProgressBar.setVisibility(View.VISIBLE); }
        else { loadingProgressBar.setVisibility(View.GONE); exoPlayPause.setVisibility(View.VISIBLE); if (focusPlay) { focusPlay = false; exoPlayPause.requestFocus(); } }
    }

    @RequiresApi(api = Build.VERSION_CODES.O)
    @Override protected void onUserLeaveHint() { if (mPrefs!= null && mPrefs.autoPiP && player != null && player.isPlaying() && Utils.isPiPSupported(this)) enterPiP(); else super.onUserLeaveHint(); }
    ...
    private void enterPiP() {
        ...
        playerView.setControllerAutoShow(false); playerView.hideController();
        final Format format = player.getVideoFormat();
        if (format != null) {
    ...
    void setEndControlsVisible(boolean visible) {
        findViewById(R.id.delete).setVisibility((visible && haveMedia && Utils.isDeletable(this, mPrefs.mediaUri)) ? View.VISIBLE : View.INVISIBLE);
        findViewById(R.id.next).setVisibility((visible && haveMedia && (nextUri != null || (mPrefs.askScope && !isTvBox))) ? View.VISIBLE : View.INVISIBLE);
    }
    ...
    void askDeleteMedia() {
        ...
        builder.setPositiveButton(R.string.delete_confirmation, (dialogInterface, i) -> {
            releasePlayer(); deleteMedia();
            if (nextUri == null) { haveMedia = false; setEndControlsVisible(false); playerView.setControllerShowTimeoutMs(-1); }
            else skipToNext();
        });
        builder.setNegativeButton(android.R.string.cancel, (dialog, which) -> {});
        builder.create().show();
    ...
    void deleteMedia() {
        try {
            if (ContentResolver.SCHEME_CONTENT.equals(mPrefs.mediaUri.getScheme())) DocumentsContract.deleteDocument(getContentResolver(), mPrefs.mediaUri);
            else if (ContentResolver.SCHEME_FILE.equals(mPrefs.mediaUri.getScheme())) {
                File file = new File(mPrefs.mediaUri.getSchemeSpecificPart());
    ...
    private void dispatchPlayPause() {
        ...
        if (state == Player.STATE_IDLE || state == Player.STATE_ENDED || !player.getPlayWhenReady()) { shortControllerTimeout = true; androidx.media3.common.util.Util.handlePlayButtonAction(player); }
        else androidx.media3.common.util.Util.handlePauseButtonAction(player);
    }

    void skipToNext() { if (nextUri != null) { releasePlayer(); mPrefs.updateMedia(this, nextUri, null); searchSubtitles(); initializePlayer(); } }
    ...
    void notifyAudioSessionUpdate(final boolean active) {
        ...
        if (active) intent.putExtra(AudioEffect.EXTRA_CONTENT_TYPE, AudioEffect.CONTENT_TYPE_MOVIE);
        try { sendBroadcast(intent); } catch (SecurityException e) { e.printStackTrace(); }
    }

    void updateButtons(final boolean enable) {
        if (buttonPiP != null) Utils.setButtonEnabled(this, buttonPiP, enable);
        Utils.setButtonEnabled(this, buttonAspectRatio, enable);
        Utils.setButtonEnabled(this, exoSettings, isTvBox || enable);
    }

    private void scaleStart() {
        isScaling = true;
        if (playerView.getResizeMode() != AspectRatioFrameLayout.RESIZE_MODE_ZOOM) playerView.setResizeMode(AspectRatioFrameLayout.RESIZE_MODE_ZOOM);
        scaleFactor = playerView.getVideoSurfaceView().getScaleX();
        playerView.removeCallbacks(playerView.textClearRunnable); playerView.clearIcon();
        playerView.setCustomErrorMessage((int)(scaleFactor * 100) + "%");
        playerView.hideController(); isScaleStarting = true;
    }

    private void scale(boolean up) {
        scaleFactor += (up ? 0.01 : -0.01);
        scaleFactor = Utils.normalizeScaleFactor(scaleFactor, playerView.getScaleFit());
        playerView.setScale(scaleFactor); playerView.setCustomErrorMessage((int)(scaleFactor * 100) + "%");
    }

    private void scaleEnd() {
        isScaling = false; playerView.postDelayed(playerView.textClearRunnable, 200);
        if (player != null && !player.isPlaying()) playerView.showController();
        if (Math.abs(playerView.getScaleFit() - scaleFactor) < 0.01 / 2) { playerView.setScale(1.f); playerView.setResizeMode(AspectRatioFrameLayout.RESIZE_MODE_FIT); }
        updatebuttonAspectRatioIcon();
    }

    private void updatebuttonAspectRatioIcon() { buttonAspectRatio.setImageResource(playerView.getResizeMode() == AspectRatioFrameLayout.RESIZE_MODE_ZOOM ? R.drawable.ic_fit_screen_24dp : R.drawable.ic_aspect_ratio_24dp); }

    private void updateButtonRotation() {
        boolean portrait = getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT;
        boolean auto = false;
        try { auto = Settings.System.getInt(getContentResolver(), Settings.System.ACCELEROMETER_ROTATION) == 1; } catch (Exception e) {}
        if (mPrefs.orientation == Utils.Orientation.VIDEO) {
            buttonRotation.setImageResource(auto ? R.drawable.ic_screen_lock_rotation_24dp : (portrait ? R.drawable.ic_screen_lock_portrait_24dp : R.drawable.ic_screen_lock_landscape_24dp));
        } else {
            buttonRotation.setImageResource(auto ? R.drawable.ic_screen_rotation_24dp : (portrait ? R.drawable.ic_screen_portrait_24dp : R.drawable.ic_screen_landscape_24dp));
        }
    }
}
